...
test.describe("Login and Logout Flow", () => {
  ...
  test("invalid login", async () => {
    try {
      ...
      await expect
        .soft(
          page.getByText(`An internal server error occurred, Please try again later. [Code: 500]
        `)
        )
        .toBeVisible();
    } catch (error) {
  ...
  test("logout after valid login", async () => {
    try {
      ...
      // Check if logout button is visible
      console.log(
        "Step 3: Verifying if the logout button is visible after successful login..."
      );
      await expect
        .soft(page.getByRole("link", { name: "Log out" }))
        .toBeVisible();
      ...
      console.log(
        "Step 5: Verifying if the login button is visible after logout..."
      );
      await expect
  ...
  test("Check for news feed is loaded properly or not ", async () => {
    ...
    const response = await context.request.get(
      "https://uat.gradientcyber.net/cyber/uauth/news/feed.json?size=5&offset=0"
    );
